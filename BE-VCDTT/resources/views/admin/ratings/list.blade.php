...
@section('meta_title')
Dánh sách đánh giá {{ $data->tour->name ?? 'test' }}
@endSection
@section('content')
            ...
            <div class="row g-2 align-items-center">
                <div class="col">
                    <h1 class="text-indigo mb-4" style="font-size: 36px;">
                        ...
                        <a href="javascript: viewDetailT({{ $data->tour->id ?? 1 }});"
                            title="Show Detail">{{ $data->tour->name ?? 'test' }}</a>
                    </h1>
                </div>
                <div class="col-12 ">
                        ...
                        <div class="card border-0 shadow-lg rounded-4 ">
                            <div class="card-header">
                                <h3 class="card-title">Trung bình đánh giá:
                                    ...
                                    @endphp
                                    {{ round($star / ($t == 0 ? 1 : $t), 1) }} <i class="fa-solid fa-star"
                                        style="color: #fffa75;"></i> (Tổng số đánh giá: {{ $t }} )
                                </h3>
                                <div class="col-auto ms-auto d-print-none">
                                    <a href="{{ url('/rating') }}" class="btn btn-default d-none d-sm-inline-block">
                                        ...
                                        Xem danh sách đánh giá của tất cả
                                    </a>
                                </div>
                            </div>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="javascript: viewDetail({{ $item->id }});"
                                                            title="Show Detail">{{ string_truncate($item->content, 40) }}</a>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td class="text-end">
                                                        @if (auth()->user()->can('reply review') || auth()->user()->is_admin == 1)
                                                            <a class="btn btn-icon btn-outline-green"
                                                                href="{{ route('rating.edit', ['id' => $item->id]) }}">
                                                                <svg xmlns="http://www.w3.org/2000/svg"
                            ...
                            <div class="card-footer d-flex align-items-center">
                                ...
                                <select id="rpp" class="form-select me-2" style="max-width: 75px;">
                                    @foreach ($pageLimits as $p)
                                        <option {{ $data->ratings->perPage() == $p ? 'selected' : '' }}
                                            value="{{ $p }}">{{ $p }}</option>
                                    @endforeach
                                </select>

                                <p class="m-0 text-secondary">Hiển thị <span>{{ $data->ratings->currentPage() }}</span>
                                    trên <span>{{ $data->ratings->lastPage() }}</span> của
                                    <span>{{ $data->ratings->total() }}</span>
                                    bản ghi</p>

                                    <ul class="pagination m-0 ms-auto">
                                        <li class="page-item {{ $data->ratings->currentPage() != 1 ? '' : 'disabled' }}">
                                            <a class="page-link" href="{{ $data->ratings->previousPageUrl() }}" tabindex="-1"
                                                aria-disabled="true">
                                        ...
                                        @if (1 + 2 != $data->ratings->currentPage() && $data->ratings->currentPage() > 4)
                                            <li class="page-item">
                                                ...
                                            </li>
                                        @endif
                                        @for ($page = max(2, $data->ratings->currentPage() - 2); $page <= $data->ratings->currentPage() + 2 && $page <= $data->ratings->lastPage(); $page++)
                                            <li class="page-item {{ $page == $data->ratings->currentPage() ? 'active' : '' }}">
                                                <a class="page-link" href="{{ $data->ratings->url($page) }}">{{ $page }}</a>
                                            </li>
                                        @endfor
                                        @if ( $data->ratings->currentPage() < $data->ratings->lastPage() - 3)
                                            <li class="page-item">
                                                ...
                                            </li>
                                        @endif
                                        @if ($data->ratings->lastPage() != 1 && $data->ratings->currentPage() < $data->ratings->lastPage() - 2)
                                        <li class="page-item {{ $page == $data->ratings->currentPage() ? 'active' : '' }}">
                                            <a class="page-link"
