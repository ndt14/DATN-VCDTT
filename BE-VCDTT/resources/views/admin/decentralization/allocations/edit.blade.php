                ...
                <div class="col-sm-12 col-md-8 offset-md-2">
                    <form id="frmAdd" class="card border-0 shadow-lg rounded-4 " action="" method="POST">
                                    ...
                                    @if (!empty($list_users))
                                        @foreach ($list_users as $user)
                                            <option value="{{ $user->id }}"
                                                {{ $user_id == $user->id ? 'SELECTED' : '' }}>{{ $user->email }}</option>
                                        @endforeach
                                    @endif
                                </select>
                                            ...
                                            <div class="col-md-3">
                                                <label class="form-check">
                                                    <input class="form-check-input" type="checkbox" name="role[]"
                                                        @if (in_array($role->id, $roles_user)) checked @endif
                                                        value="{{ $role->id }}">
                        ...
                        <div class="card-footer text-right">
                            <button id="btnSubmitAdd" type="button" data-id="{{ $user_id }}"
        ...
        $(document).ready(function() {
            ...
            $('#btnSubmitAdd').click(function(e) {
                ...
                $.ajax({
                    ...
                    url: "{{ route('allocation.edit', ['user_id' => ':id']) }}".replace(':id', this
                        .dataset.id),
                    type: "POST",
                    data: formData,
                    contentType: false,
